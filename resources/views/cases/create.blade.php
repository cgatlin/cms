...
<div class="flex items-center justify-center text-base-content">
    <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 mt-2" action="/cases" method="POST">
        <h1 class="block text-sm font-bold mb-2">Create New Case:</h1>
        @csrf

            <label class="block text-gray-700 text-sm font-bold mb-2" for="title">
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" type="text" id="title" name="title" placeholder="Title:" required>
            </label>
            @if ($errors->has('title'))
            ...
            <label class="block text-gray-700 text-sm font-bold mb-2" for="description">
                <textarea class="textarea shadow appearance-none border border-black rounded w-full py-2 px-3 bg-white text-gray-700" type="text" id="description" name="description" placeholder="Description:" required></textarea>
            </label>
            ...
            <label class="block text-gray-700 text-sm font-bold mb-2" for="client">
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700"  type="text" name="client" id="client" list="clients" placeholder="Type to search for client...">
                <datalist id="clients">
            ...
            <label class="block text-gray-700 text-sm font-bold mb-2" for="status">
                <select class="select shadow appearance-none border border-black rounded w-full py-2 px-3 bg-white text-gray-700" name="status" id="status" required>
                    <option value={{ CaseRecordsStatus::OPEN }}>{{ CaseRecordsStatus::OPEN->label() }}</option>
                    <option value={{ CaseRecordsStatus::IN_PROGRESS }}>{{ CaseRecordsStatus::IN_PROGRESS->label() }}</option>
                    <option value={{ CaseRecordsStatus::CLOSED }}>{{ CaseRecordsStatus::CLOSED->label() }}</option>
                </select>
            </label>
